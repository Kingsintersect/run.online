...
import type { SystemLogsQueryParams } from "@/services/configurationApi"
...
export function useUpdateSetting() {
  ...
  return useMutation({
    ...
    onSuccess: async (
      _,
      variables: { id: number; payload: UpdateSettingPayload }
    ) => {
      await Promise.all([
        qc.invalidateQueries({ queryKey: configurationKeys.settings() }),
        qc.invalidateQueries({
          queryKey: configurationKeys.settingDetail(variables.id),
        }),
      ])
    },
  })
...
export function useScheduledJobs() {
  return useQuery({
    ...configurationQueryOptions.scheduledJobs(),
    staleTime: 1000 * 60,
  })
}

export function useSystemLogs(params?: SystemLogsQueryParams) {
  return useQuery({
    ...configurationQueryOptions.logs(params),
    staleTime: 1000 * 15,
  })
}
