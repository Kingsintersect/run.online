...
export const useGSAPAnimations = () => {
  ...
  /**
   * Slides ship with `opacity-0` and are only ever revealed by GSAP, so the
   * first one has to be shown explicitly on mount — otherwise the hero stays
   * blank until the first transition fires.
   */
  const revealSlide = useCallback((slide: HTMLElement) => {
